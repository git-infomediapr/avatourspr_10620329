...
export interface RippleDistortionProps {
  ...
  className?: string;
  style?: CSSProperties;
  onTextureLoad?: (src: string) => void;
  onTextureError?: (src: string) => void;
}
...
const RippleDistortion = ({
  ...
  const callbacksRef = useRef({ onTextureLoad, onTextureError });
  callbacksRef.current = { onTextureLoad, onTextureError };
  const textureApiRef = useRef<{
  ...
  useEffect(() => {
    ...
    const loadImage = (url: string) => {
      ...
      image.onload = () => {
        ...
        try {
          ...
          callbacksRef.current.onTextureLoad?.(url);
        } catch {
          setCanvasVisible(false);
          callbacksRef.current.onTextureError?.(url);
        }
      };
      image.onerror = () => {
        if (disposed || token !== loadToken) return;
    ...
    textureApiRef.current = { load: loadImage };
  ...
  useEffect(() => {
    textureApiRef.current?.load(src);
  }, [src]);
  ...
  return (
    <div
      ref={mountRef}
      className={`relative h-full w-full overflow-hidden bg-black pointer-events-none [&>canvas]:block [&>canvas]:h-full [&>canvas]:w-full ${className}`.trim()}
      style={style}
      aria-hidden="true"
    />
  );
